...
locals {
  user_data = <<-EOF
    ...
    log_with_timestamp "Cloning CloudMart Infrastructure Repository" "
      ...
      timeout 120 git clone https://github.com/jrmreis/cloudmart-infrastructure.git /home/ec2-user/cloudmart-infrastructure
      ...
      if [ \$? -ne 0 ]; then
        ...
        timeout 120 git clone --depth 1 https://github.com/jrmreis/cloudmart-infrastructure.git /home/ec2-user/cloudmart-infrastructure
        
        if [ \$? -ne 0 ]; then
          echo 'Second clone attempt failed, trying with git archive...'
          
          # Try with git archive as a last resort
          mkdir -p /home/ec2-user/cloudmart-infrastructure
          curl -L https://github.com/jrmreis/cloudmart-infrastructure/archive/main.tar.gz | tar -xz -C /home/ec2-user/cloudmart-infrastructure --strip-components=1
      ...
      chown -R ec2-user:ec2-user /home/ec2-user/cloudmart-infrastructure
    "
...
EOF2"
    ...
    log_with_timestamp "Setting File Ownership" "chown -R ec2-user:ec2-user /home/ec2-user/challenge-day2 && chown -R ec2-user:ec2-user /home/ec2-user/cloudmart-infrastructure"
...
resource "aws_instance" "workstation" {
  ...
  # Add CloudWatch agent for advanced monitoring
  # The CloudWatch agent requires additional setup that would be done in user_data
}
